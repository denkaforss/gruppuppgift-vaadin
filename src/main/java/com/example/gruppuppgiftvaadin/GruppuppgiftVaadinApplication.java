...
public class GruppuppgiftVaadinApplication implements AppShellConfigurator {
    ...
    CommandLineRunner init(ArtistRepo artistRepo, AlbumRepo albumRepo, SongsRepo songsRepo, AppUserRepo appUserRepo) {
        return args -> {
            ...
            Artist artist = new Artist("first artist", LocalDate.of(2001, 1, 1), "sweden", appUser);
            artistRepo.save(artist);

            Album album = new Album("first album", LocalDate.of(2002, 2, 2), artist, appUser);
            album.setArtist(artist);
            albumRepo.save(album);

            Songs songs = new Songs("first song", 149, artist, album, appUser);
            album.setArtist(artist);
            album.setAlbumName(album.getAlbumName());
            songsRepo.save(songs);

            Artist artist2 = new Artist("second artist", LocalDate.of(2001, 1, 1), "denmark", appUser);
            artistRepo.save(artist2);

            Album album2 = new Album("second album", LocalDate.of(2002, 2, 2), artist2, appUser);
            album.setArtist(artist2);
            albumRepo.save(album2);

            Songs songs2 = new Songs("second song", 149, artist2, album2, appUser);
            album.setArtist(artist2);
